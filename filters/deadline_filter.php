<?php
	...
	if (!isset($_SESSION['user_id'])) {
		...
		$query = sprintf("SELECT * FROM Tasks WHERE User_ID = '$User_ID' and Status = 2 Order by Deadline"); 
		...
		while ($task = mysql_fetch_assoc($result)) {
			...
			$deadline = date_format($datetimeobj, 'M d');
			$time = date_format($datetimeobj, 'g:ia');
			echo "<div class='task hide_task' id='$task_id'><div class='task_name hidden_task_name'>".
					"<ul class='deadline'><li class='month_date'>".$deadline."</li>".
					"<li class='time'>".$time."</li></ul>  ".$task["Name"].
					"</div>".
